package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(var_405.getValue(_loc4_) == null)
               {
                  var_405.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(var_1042.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(var_1043.indexOf(_loc6_) == -1)
               {
                  ...
                  var_263.push(_loc8_);
                  var_1043.push(_loc6_);
                  if(param3 != null && var_715.indexOf(param3) == -1)
                  {
                     var_715.push(param3);
                  }
               }
      ...
      {
         var _loc3_:IPetData = var_405.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      private function notifyListeners() : void
      {
         ...
         while(false)
         {
            _loc1_ = var_715.pop() as IPetDataListener;
            if(_loc1_ != null)
            {
               _loc1_.petDataReady();
            }
      ...
      {
         var _loc4_:PetData = var_405.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = var_405.getKeys();
         for each(key in speciesKeys)
         {
            petData = var_405.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_1934.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
