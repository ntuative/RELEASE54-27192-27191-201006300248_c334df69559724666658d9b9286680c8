package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private var var_2167:MarketplaceItemStats;
      ...
      {
         if(!var_164 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_164.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         showConfirmation(const_1590,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_2165 || param1.furniTypeId != var_2164)
         {
            return;
         }
         var_2167 = param1;
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = var_217.remove(parser.offerId);
            if(item != null)
            {
               item.dispose();
            }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = var_164.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = var_164.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_164.add(parser.offerId,updateItem);
            }
            var_164.remove(parser.requestedOfferId);
            showConfirmation(const_1655,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_217.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_217.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_443)
            {
               var_217.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_1844 = _loc3_.creditsWaiting;
         var_217 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_217.add(_loc4_.offerId,_loc6_);
         }
      ...
      private function showConfirmation(param1:int, param2:MarketPlaceOfferData) : void
      {
         if(!var_211)
         {
            var_211 = new MarketplaceConfirmationDialog(this,_catalog,_roomEngine);
         }
         var_211.showConfirmation(param1,param2);
      }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_164.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_2163,var_2162,var_2161,var_2166);
      }
