package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class BotCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid, IAvatarImageListener
   {
      ...
      private var var_1424:Map;
      
      private var var_799:Offer;
      ...
      public function avatarImageReady(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc8_:* = null;
         var _loc2_:int = var_1669.getValue(param1);
         if(true)
         {
            return;
         }
         for each(_loc3_ in page.offers)
         {
            ...
            {
               _loc4_ = var_1424.getValue(_loc3_.productContainer.firstProduct.extraParam.toLowerCase());
               ...
               if(_loc7_)
               {
                  _loc7_.setDirection(AvatarSetType.const_29,4);
                  _loc8_ = _loc7_.getImage(AvatarSetType.const_29,true);
                  _loc6_.bitmap.copyPixels(_loc8_,_loc8_.rect,new Point((_loc6_.width - _loc8_.width) / 2,_loc6_.height - _loc8_.height));
                  _loc6_.invalidate();
                  _loc7_.dispose();
                  if(var_799.offerId == _loc2_)
                  {
                     updatePreviewImage();
                  }
               }
      ...
      protected function createGridItem(param1:Offer) : void
      {
         ...
         var _loc3_:String = var_1424.getValue(param1.productContainer.firstProduct.extraParam.toLowerCase());
      ...
      {
         var _loc1_:String = var_1424.getValue(var_799.productContainer.firstProduct.extraParam.toLowerCase());
         ...
         if(_loc2_)
         {
            _loc2_.setDirection(AvatarSetType.const_29,4);
            setPreviewImage(_loc2_.getImage(AvatarSetType.const_29,true));
            _loc2_.dispose();
         }
      }
      ...
      {
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_799,page);
      }
      ...
      private function setProductData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc1_.registerParameter("catalog.purchase.price.credits+pixels","credits",String(var_799.priceInCredits));
            _loc1_.registerParameter("catalog.purchase.price.credits+pixels","pixels",String(var_799.priceInPixels));
            _loc2_.text = "${catalog.purchase.price.credits+pixels}";
         }
         var _loc3_:IProductData = page.viewer.catalog.getProductData(var_799.localizationId);
