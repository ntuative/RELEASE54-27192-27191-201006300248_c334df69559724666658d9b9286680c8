package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_448 >= var_329.length)
         {
            return "";
         }
         var petColor:IPetColor = var_329[var_448] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_665[var_1633] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_448 = param1.index;
         if(var_448 < 0 || var_448 > var_329.length)
         {
            var_448 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_627.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_126,onTextWindowEvent);
         var_627 = new Map();
         ...
         var_952 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_329 = [];
         var_665 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_952,this);
               if(_loc11_)
               {
                  var_665 = _loc11_.breeds;
                  var_329 = _loc11_.colors;
                  if(var_329 == null)
                  {
                     var_329 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_665.length)
         {
            _loc12_ = var_665[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_952,_loc12_.id),getRaceLocalizationKey(var_952,_loc12_.id)));
            _loc6_++;
         }
         var_627.add(var_952,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1050,onApproveNameResult);
         _loc5_ = var_627.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_329)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_627.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_448 >= 0 && var_448 < var_329.length)
         {
            _loc7_ = var_329[var_448] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_665[var_1633] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_952,_loc3_.id,_loc2_,AvatarScaleType.const_49,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_275,AvatarAction.const_539);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_29,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_29),true);
         _loc4_.dispose();
