package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String)
      {
         ...
         _assets = param1;
         var_1404 = param2;
         var_1403 = "http://" + param3 + "/client";
         var_1404.init();
         var_1404.addEventListener(HabboWeb.const_197,requestClientURL);
      }
      ...
      {
         var _loc2_:String = var_1403;
         if(_assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + var_1403);
         }
         var _loc3_:URLRequest = new URLRequest(var_1403);
         if(_assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            var_1404.requestReAuthenticate();
         }
         else
         {
            try
            {
               var_1644 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               var_2159 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               if(false)
               {
