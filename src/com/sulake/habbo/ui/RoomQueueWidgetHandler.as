package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_386:
               ...
               {
                  case RoomSessionQueueEvent.const_990:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_962:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_1046) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1046) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1327) + 1;
                  }
               }
               else
               {
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_750:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_962);
               break;
            case RoomWidgetRoomQueueMessage.const_619:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_990);
               break;
            case RoomWidgetRoomQueueMessage.const_738:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_245,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_386];
      }
